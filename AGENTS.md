...
- **网页端的文件读取**：统一入口在 `web/src/document.ts`（按 MIME＋扩展名分发，各读取器按需加载）。PDF 走 `web/src/pdf.ts`（pdfjs），Word 走 `web/src/docx.ts`（mammoth 的 `extractRawText`，产出"一段一行"）；坐标重组行的逻辑在引擎的 `src/text-lines.ts`（纯函数、有单测）。**接新格式时的分工**：把"文件 → 文本行"放在 `web/`（那里才有库依赖），把"文本行 → 条款"留给引擎（`extract-contract.ts` 已足够宽容）。
  ...
  - **扫描件与拍照图片读不出文字**，这条路径必须有明确报错，不能产出空报告。**`.doc` 与图片／扫描件是作者明确划出的范围边界，不要再接**：`.doc` 是 OLE 二进制、浏览器端要读它是独立的一块工程；图片要中文 OCR，浏览器端没有可靠又免费的方案，硬撑会违背"宁可报读不出来，也不给错报告"的取舍。要做的只是把原因说清楚并给出替代做法（另存为 `.docx` ／ 复制或手工录入条款）。
...
- `NUMERIC_COMPARE` / `RATIO_COMPARE` / `TIERED_COMPARE` 依赖 `src/extract-facts.ts` 抽出的事实。三条约束不要放松：
  1. **抽不到就是 `value: null`，不是 0，也不算合规。** 引擎据此产出 `undetermined`（无法判定），这是与"违规""合规"并列的第三种结果。不要为了让结果好看而给事实填默认值。
     **但"合同明确写了本项不约定"要用 `method: 'NOT_AGREED'` 表达，不要用 `UNRECOGNIZED` 顶替**：前者规则**不适用**（不报违规，也不列进"无法判定"），后者才是"无法判定"。把确定的事说成"没查出来"是另一种误导——真实反馈：「合同明确不约定竞业限制义务」被列进了"无法判定：竞业限制期限超过两年"。关键信息里这一档显示这句话本身。
...
- **构建产物必须可复现**：产物里唯一的非确定项是 `provenance.generatedAt`，它必须走 `src/build-time.ts` 的 `buildTimestamp()`（会读 `SOURCE_DATE_EPOCH`），**不要直接写 `new Date().toISOString()`**。实测同一份快照两轮构建逐字节一致；要验证就设一个固定的 `SOURCE_DATE_EPOCH` 连续构建两次比对哈希。这个开关是"产物能从快照重建"这句话的唯一凭据。
