...
- **报告层**（`src/score.ts` / `src/build-report.ts`）三条不可放松的约束：
  1. **评分必须能解释**：分数 = 100 − 各等级扣分之和（下限 0），高危项一票封顶。报告里要列出每一笔扣分，不要改成黑盒加权。
  2. **无法判定的项单独成块、且不计分**——没抽到信息既不加分也不扣分，但必须让用户看到。绝不能混进"没问题"里。
  3. **关键信息的措辞要区分"有值"与"只是提到了"**。没有事实键的项只能判断合同是否提及，措辞必须写明"未核对具体内容"，否则等于在暗示已经查过了。
  改 `RISK_LEVEL_PENALTY` 或 `CAP_RULE_CODES` 会改变所有报告的分数，改前想清楚并同步更新 `tests/build-report.test.ts`。
...
- **网页端在 `web/`，是纯前端应用**（Vite + React + TS）。两条约束：
