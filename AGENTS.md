...
- **网页端的文件读取**：统一入口在 `web/src/document.ts`（按 MIME＋扩展名分发，各读取器按需加载）。PDF 走 `web/src/pdf.ts`（pdfjs），Word 走 `web/src/docx.ts`（mammoth 的 `extractRawText`，产出"一段一行"）；坐标重组行的逻辑在引擎的 `src/text-lines.ts`（纯函数、有单测）。**接新格式时的分工**：把"文件 → 文本行"放在 `web/`（那里才有库依赖），把"文本行 → 条款"留给引擎（`extract-contract.ts` 已足够宽容）。
  ...
  - **文本"读得出来"不等于"读得对"**，提取后必须过 `src/text-lines.ts` 的 `assessExtractedText`：它会①按 Unicode 等价映射修回被字体映射成部首的字形（`甲⽅` → `甲方`，见 `src/cjk-radicals.ts`）②剥掉控制字符③判断这份文本能不能用。改判据时注意：**不要拿"控制字符总占比"当判据**（正常 PDF 每行尾都可能带一个 SOH），要看夹在正文中间的那些；也**不要整篇 NFKC**（会改掉全角标点，报告引用的原文就不原样了）。
...
- `NUMERIC_COMPARE` / `RATIO_COMPARE` / `TIERED_COMPARE` 依赖 `src/extract-facts.ts` 抽出的事实。三条约束不要放松：
  ...
  6. 事实键定义在 `src/schema.ts` 的 `FACT_KEYS`（rule 的 `field`/`ratioOf`/`dependsOn` 只能取这些值，写错键名 schema 会拦下）。新增事实要同时改 schema、`extract-facts.ts`、`build-report.ts` 的 `KEY_INFO_ITEMS` 与 `FACT_META`。
