...
- **报告层只做标注，不做评价**（`src/build-report.ts`）。三条不可放松的约束：
  ...
  3. **关键信息每一项都要取真值，取不到值才退回"提及"判断，且必须把原文摘出来。** 状态只有四种：`VALUE`（数值）、`TEXT`（文本事实，值就是合同原句）、`MENTIONED`（有相关字样但没识别出内容，带原文片段）、`NOT_FOUND`。**不要退回"有提及（未核对内容）"这种含糊说法**——等于什么都没说，还会让人误以为已经核对过了。每一项的 `mention` 关键词只用于"取不到值时区分有提及/未提及"，**绝不能拿它填值**。
- **隐私是硬约束，不是宣传语。** 三条不要破坏：
...
- **网页端的文件读取**：统一入口在 `web/src/document.ts`（按 MIME＋扩展名分发，各读取器按需加载）。PDF 走 `web/src/pdf.ts`（pdfjs），Word 走 `web/src/docx.ts`（mammoth 的 `extractRawText`，产出"一段一行"）；坐标重组行的逻辑在引擎的 `src/text-lines.ts`（纯函数、有单测）。**接新格式时的分工**：把"文件 → 文本行"放在 `web/`（那里才有库依赖），把"文本行 → 条款"留给引擎（`extract-contract.ts` 已足够宽容）。
  - 验证文件上传：这个 MCP 没有 `browser_file_upload`，而 `#pdf-input` 是隐藏的、`browser_drop` 会被 `.actions` 挡住。可用做法是在页面里 `fetch('/@fs/<仓库绝对路径>')` 取字节 → `new File(...)` → 塞进 `#pdf-input.files` → 派发 `change` 事件（Vite 的 `server.fs.allow: ['..']` 允许读仓库内的文件）。测试用文件放在 `tmp/`（已 gitignore，不进仓库）；PDF 可从政府网站抓，`.docx` 范本可从省市人社厅网站的附件里找。
  - **扫描件与拍照图片读不出文字**，这条路径必须有明确报错，不能产出空报告——这是本地化架构的硬代价，不要用低准确率的 OCR 硬撑。
  - **文本"读得出来"不等于"读得对"，而且两种格式必须走同一道关卡。** 读取器（`pdf.ts` / `docx.ts`）只负责产出"一行一段"的文本；码位修复、控制字符清洗、可用性判定统一放在 `web/src/document.ts` 里调 `src/text-lines.ts` 的 `assessExtractedText`——**不要把它写进某一个读取器**，只在一条路径上做等于给另一条埋静默错误（Word 路径一开始就是这么漏掉的）。改判据时注意：**不要拿"控制字符总占比"当判据**（正常 PDF 每行尾都可能带一个 SOH），要看夹在正文中间的那些；也**不要整篇 NFKC**（会改掉全角标点，报告引用的原文就不原样了）。
  - 文件读不出来（加密、损坏、把 `.doc` 改成 `.docx`）时要给**可操作的提示**，不要把库的英文异常原样抛给用户。
...
- `NUMERIC_COMPARE` / `RATIO_COMPARE` / `TIERED_COMPARE` 依赖 `src/extract-facts.ts` 抽出的事实。三条约束不要放松：
  ...
  3. **候选范围只能是"被选中的那一项 + 选项清单之前的共用文字"**（`applicableScopes`），**绝不包含没被选中的选项**。只取"被选中的项"会漏掉写在选项清单之前的内容（例如「于每月 X 日前足额支付」里的发薪日），退回整条原文又会把未选中的数额报出来——两个坑都踩过。
  4. **文本类事实取标签所在的整句**（遇句号/分号/换行即停），不要从标签往后硬截固定长度，否则会摘出跨句的半截原文。
  5. **日期区间按"含首尾两天"算月数。** 合同里的「自 X 起至 Y 止」含终止日当天，2026-09-15 至 2029-09-14 就是整三年。按"经过时长"算会得到 35 个月，落进《劳动合同法》第十九条「一年以上不满三年」那一档，把合法的 6 个月试用期误判成超长。
  6. **标题不是内容。** 章节标题（「四、违约责任」「工作内容和工作地点」「劳动报酬及支付」）会命中同一批关键词，但它们不是"合同对这个事项的约定"。文本类事实取值要跳过标题（`looksLikeHeading`）；"仅提及"的摘录也要排除标题，并优先取**命中本行关键词最多**的那句（同样多时取最短）。
  7. 事实键定义在 `src/schema.ts` 的 `FACT_KEYS`（rule 的 `field`/`ratioOf`/`dependsOn` 只能取这些值，写错键名 schema 会拦下）。新增事实要同时改 schema、`extract-facts.ts`、`build-report.ts` 的 `KEY_INFO_ITEMS` 与 `FACT_META`。
- 改比对引擎或事实抽取后必须跑 `npm run eval:diff`：它除了验四类差异，还用**真实范本数据**（不是手写夹具）跑一遍事实抽取，任何一项抽出"未识别"都会失败。手写夹具会掩盖真实文本的形态问题——已经踩过一次（相邻填空位未合并）。
- `eval:diff` 里还有两道闸门：① **把规则跑在官方范本派生的示例合同上，要求报出 0 条风险项**（范本本身是合规的，报出来就是关键词放宽过头导致的误报——为了修漏报而放宽关键词后，务必看这道闸门有没有亮）；② **`MUST_HAVE_VALUE` 列出的关键信息必须取出值**（范本里写明了数额/期限却只判成"仅提及"，说明抽取规则在真实文本上失效了）。这两条是防"修一个漏报、制造十个误报"和防"抽取静默退化"的唯一自动手段。
