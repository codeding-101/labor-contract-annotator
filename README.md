...
src/
  ...
  score.ts             已删除（早期版本的评分模块，按产品定位移除）
...
引擎的三块输出（差异清单、标注项、事实）在这里组装成一份用户能看的报告。三条设计约束：
...
- **结果页按信息层级排**：标注计数概览 → 标注的条款（每条含合同原文、风险说明、判断依据、法条原文、建议）→ 无法判定 → 与范本的差异 → 关键信息 → 三段总结 → 免责声明。
...
- **界面只在 Chromium 内核里验证过**。所谓"桌面 1280×900 + 移动 390×844"是**视口模拟**，不是真机。Firefox / Safari 没测过；iOS 上的文件选择行为、以及 `connect-src 'none'` 这条 CSP 在 Safari 下与 pdfjs worker 的配合，都还是未知。
