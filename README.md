...
面向应届生秋招签约场景。设计文档见 [`docs/product-design.md`](docs/product-design.md)。采用 [Apache-2.0](LICENSE) 许可。
...
# 可复现构建：固定产物里的构建时间戳，两轮构建逐字节一致（Windows 用 $env:SOURCE_DATE_EPOCH = "…"）
SOURCE_DATE_EPOCH=1789000000 npm run check
```
...
**2. 产物可复现，而且可验证。** 实测同一份快照连续构建两次，条目、章节、案例与快照哈希**逐字节一致**，唯一的非确定项是产物里的 `provenance.generatedAt`（构建时间）。所以要证明"这份数据能从快照重建"，只用一句承诺不够，得有个开关：设了 `SOURCE_DATE_EPOCH`（可复现构建的通行约定，Unix 秒）就固定这个时间戳。实测固定后两轮构建 7 个产物哈希完全相同；值写错时**直接报错**，不悄悄退回当前时间——否则会给出一个"看起来可复现、其实不可复现"的产物。
...
**3. 双来源交叉校验。** 同一部法律取两个独立官方来源，归一化标点与空白后逐条比对。主来源要求抽取干净（条号连续、无抽取问题），交叉来源不要求——它存在的意义就是暴露主来源的问题。差异条号会被记录并打印，供人工复核。
...
**4. 结构断言。** 条号必须严格连续（从第一条开始，遇到断号立即停止并报错，宁可少收不可错收）；条文正文不得过短；条文 ID 与条号必须一致。主来源断言失败时**跳过写出**，不产出可疑数据。
...
**3. 关键信息 17 项全部取真值，取不到值就摘原文。** 状态有五种：`有值`（抽到数值，带单位）、`原文`（文本类事实，值就是合同原句）、`明确不约定`（合同写了本项不约定，值就是这句话）、`有相关约定但未识别出内容`（附原文片段供人工核对）、`合同未提及`。这里原先有 11 项只做关键词存在性判断、显示成"有提及（未核对内容）"——等于什么都没说，而且关键词表覆盖不到真实写法时还会误报"合同未提及"。**"合同未提及"本身就是一条提示**（法定必备条款缺失的另一种视角）。
...
| 验证项 | 结果 |
|---|---|
| 上传真实 PDF | 一份 4 页的劳动合同（合法标准版）：提取 2505 字（修正 328 个错位字符、剥掉 107 个控制字符）→ 识别 12 条 → 0 条风险、0 项无法判定；关键信息 17 项里 12 项取到真值或原文（36 个月、6000 元、4800 元、4500 元、1500 元、15 日、8 小时、工作地点等），1 项明确标为"合同明确不约定竞业限制义务"，其余 4 项如实标为"有相关约定但未识别"或"合同未提及" |
...
| 合规示例 | 0 条风险；关键信息 9 项取到真值、6 项如实标成"有相关约定但未识别"、2 项"合同未提及" |
...
**站点已上线并实测**：<https://codeding-101.github.io/labor-contract-annotator/>
...
- **规则库 10 条，覆盖仍有限。** 已含文本类 7 条与数值类 3 条；"低于当地最低工资""加班费计算基数""经济补偿计算"等还需要先把工资支付暂行规定与地方最低工资数据入库，才能把规则绑到条文上。
...
- **社会保险法的条文已接入规则依据**：`WAIVE_SOCIAL_INSURANCE` 引用《劳动合同法》第二十六条与《社会保险法》第十二条、第六十条，`MISSING_SOCIAL_INSURANCE_TERM` 引用第十七条与第五十八条。
...
- **官方范本普遍是二进制 `.doc`**（这是"范本入库"那条构建链路的事，与网页端不做 `.doc` 上传是两回事）。北京、上海、广东、河南、沈阳、浙江、福州的范本基本都是 `.doc` 附件，网页正文里没有条款内容。`word-extractor` 已接入并能读出正文，但它对段落边界的还原不可靠（实测某市范本把"注意事项"六条连成一行），接入前需要先验证段落边界。`.docx` 范本同理：构建侧还没有对应读取器（`ContractTemplate` 的 `format` 目前只允许 `html` 与 `doc`）。
...
采用 [Apache-2.0](LICENSE)：可自由使用、修改、分发与商用，需保留版权与许可声明，不提供任何担保。

> 这个工具只做**事实比对与条文引用**，不构成法律意见；`LICENSE` 里的免责条款与报告里的免责声明都指向同一件事。涉及具体争议请咨询执业律师或当地劳动人事争议仲裁机构。
