...
| 案例语料 | 完成（v0）：2 份案例文档（最高法 6 个＋安徽高院 9 个），为规则提供带裁判结果的真实措辞 |
...
每条规则还**自带正反例**（`cases`），`npm test` 会逐条跑过。改规则必须同时改用例，否则测试失败。当前 9 条规则、34 条用例，其中 3 条标注了来源案例。

## 案例语料：用真实措辞验证规则

企业劳动合同不会公开流传，所以"用真实合同验证"这条路走不通。但它有替代品：**人社部与各级法院公开发布的典型案例**——案例会原文描述企业与劳动者之间的约定，而裁判结果就是标签。

目前收了 2 份：**最高人民法院 2025-08-01 发布的劳动争议典型案例**（6 个）与**安徽省高院、省人社厅、省总工会 2026-04-30 联合发布的典型案例**（9 个）。走的还是同一套管线：冻结快照 → 抽取（案例序号／标题／基本案情／裁判结果／典型意义）→ 结构化入库，带哈希与出处。

两份文档的形态差别不小，抽取器两种都吃下了：

| | 最高法那份 | 安徽那份 |
|---|---|---|
| 案例标题 | 「案例一」单独成块，标题在下一块 | 「案例一：标题」内联 |
| 目录 | 条目带冒号，正文不带 | **条目与正文形态完全相同** |
| 小标题 | 【基本案情】【裁判结果】【典型意义】 | 判例类同上；仲裁调解类用【基本情况】【处理过程】【处理结果】 |

两个关键处理：目录**不能靠冒号区分**，改用前视规则——正文案例在下一个案例标题之前会出现【…】小标题，目录条目不会（这和范本章节识别是同一个技巧）；小标题名称按语义归位到三个槽位，**认不出的小标题名称会报出来**（内容并入上一个已知槽位不丢），逼我补映射而不是悄悄吞掉。
...
三处修法都不是加特例，而是补上通用那一层：社保规则加了 `params.about`（要求条款"是关于社会保险的"，再在条款内匹配否定／转嫁表述），数字表补上「两」。
