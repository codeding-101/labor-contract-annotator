...
src/
  ...
  diff-template.ts     范本比对：三级对齐 + 四类差异
  score.ts             透明评分（按等级扣分 + 高危一票封顶）
  build-report.ts      组装报告（关键信息、三段总结、签署建议、免责声明）
...
引擎的三块输出（差异清单、风险项、事实）在这里组装成一份用户能看的报告。三条设计约束：

**1. 评分完全透明，不做黑盒。** 分数 = 100 − 各项扣分之和（下限 0），红 15／黄 5／蓝 1；命中特定高危项（违法违约金、约定放弃社保、工资异议期）时**一票封顶 40 分**。报告里会把这笔账直接写出来：

```
评分明细：100 − 15（约定由劳动者承担违约金） − 15（约定放弃社会保险） − 15（约定工资异议期） = 40
⚠ 命中高危项「约定由劳动者承担违约金（超出法定情形）」，总分上限 40
```

一票封顶是为了避免两种失真：一堆小问题把分数拖到很低，或者一个致命问题因为别处干净而显得分数很高。

**2. 无法判定的项单独成块，不算合规也不算风险。** 它们**不计分**——没抽到信息既不该加分也不该扣分，但必须在报告里明确列出来。这是事实层那条约束（抽不到就是 `null`）在报告层的延续：用户需要知道"这条我没能判断"，而不是以为已经查过了。

**3. 关键信息区分"有值""有提及""未提及""未识别"四种状态。** 有事实键的给出数值（合同期限 36 个月、试用期 3 个月、月工资 8000 元），没有事实键的只判断"合同有没有提到"，并且措辞上明确写"仅表示提到了，未核对具体内容"——不让人误以为已经查过了。**"合同未提及"本身就是一条提示**（法定必备条款缺失的另一种视角）。

报告自带免责声明：不构成法律意见、评分不代表法律评价、**合同中出现但本工具未覆盖的事项不在检查范围内**。

`npm run report:sample` 会把这份报告打印出来，看的是两份：一是官方范本派生的合规示例（应得满分、零风险），二是往里面注入三条典型问题条款后的版本。它同时兼任自检——分数没下降或问题条款没被报出来都会失败。
...
- **结果页按信息层级排**：评分与建议 → 风险事项（每条含合同原文、风险说明、判断依据、法条原文、建议）→ 无法判定 → 与范本的差异 → 关键信息 → 三段总结 → 免责声明。

**输入方式**：两路——直接粘贴文字，或上传**带文字层**的 PDF（电子合同基本都是这形态）。读不出文字时界面会明确说明原因并给出替代做法，**不会给出一份空报告**。
...
- 这段坐标重组逻辑放在引擎的 `src/text-lines.ts`（纯函数、有单测），`web/src/pdf.ts` 只负责 pdfjs 的接线。将来接 Word、OCR 时，只要它们能产出"文本片段（或纯文本行）"，就能复用同一条链路。
- **pdfjs 按需加载**：首屏主包 147 KB（gzip），PDF 相关代码单独成块（129 KB gzip），只有真上传 PDF 时才拉进来。

浏览器实测覆盖（桌面 1280×900 + 移动 390×844）：

| 验证项 | 结果 |
|---|---|
| 上传真实 PDF | 一份 24 页的中文 PDF → 提取 10797 字 → 识别出 98 条 → 出报告，无崩溃 |
| 上传无文字层的 PDF | 明确报错（说明多半是扫描件/拍照件）、输入框保持为空、按钮保持禁用 |
| 合规示例 | 100 分、0 条风险、差异 0 处，显示"逐条比对后没有发现差异" |
| 可疑示例 | 40 分（一票封顶），3 条严重风险全部报出，法条原文正确展示（含《劳动法》第五十条、《社会保险法》第十二/六十条） |
| 无条款编号的文本 | 退化为按段落切分，规则与事实抽取照常工作 |
| 空态 | 「开始检查」禁用、清空后报告消失、提示文案正确 |
| 移动端 | 无横向溢出 |
| 控制台 | 无错误 |

（那份用来实测的 PDF 是政府网站上的法律文本，不是真实劳动合同——企业合同拿不到，这一点见"已知局限"。）
...
- **事实抽取只覆盖 5 项，且靠正则模式。** 合同写法千变万化（"月薪八千""年薪制""综合计算工时"），现有模式命中不了的会落到"未识别"。这是有意的取舍：宁可报无法判定，也不拿猜出来的数字下结论。
- **文件格式只接了 PDF 的文本层**。Word（.docx / .doc）与图片还没接；**扫描件与拍照图片读不出来**——浏览器端没有可靠又免费的中文 OCR 方案，这恰恰是"本地化"架构最硬的一处代价。当前的做法是明确报错并引导用户改用其他方式，而不是硬撑一个准确率很低的 OCR。这是本工具最需要继续投入的一块。
- **把非合同文本喂进来会误报**。实测把一份法律条文 PDF 放进去，"约定放弃社会保险"这条规则被命中了——因为法条正文里本来就写着"不缴纳社会保险费"这些字样。本工具假定输入是**劳动合同**；对法条、政策文件这类文本，关键词规则会产生误报。
- **网页端尚未部署**。静态产物已经能直接托管，但放哪儿（GitHub Pages / 国内静态托管是否需要备案）没有定，因此简历上暂时没有可点的活链接。
- **界面样式是手写 CSS**，没有按设计文档的原计划引入 Tailwind + shadcn/ui——v0 为减少构建配置先手写，样式集中在 `web/src/styles.css`，要换随时可换。
- **关键信息里只有 5 项是真值**，其余 12 项只判断"合同是否提及"，没有核对内容。要给出真值就得继续补事实键（发薪日期、工时制度类型、公积金、社保缴费基数等）。
- **评分权重是我定的，不是法律或行业标准**（红 15／黄 5／蓝 1，高危封顶 40）。它的作用是让风险数量可比、可解释，不代表严重程度的客观度量。
- **报告只在范本派生的示例合同上自检过**（合规版满分零风险、注入问题后分数下降），没跑过真实合同——与下面那条验证来源限制同源。
